...
### 2026-09 — 5-task batch (locale architecture, intro cleanup, per-section categories, blocklist, /admin chooser)

- **Task 3 — locale reset bug FIXED at the state layer.** `LangContext.jsx` is now the ONE source of truth: it resolves the language exactly once at first paint (URL `?lang=` set by the bot → Telegram client language → English) and every screen reads `useLang().t`. The old per-screen fallbacks are gone: `AllDone`, `ConfirmOrder`, `PaymentFrame` were **100% hardcoded English** (the actual bug — UI silently reverted after picking a category); `LocationName`, `CategoryMenu`, `MainMenu`, `ServiceChoice`, `IntroFrame` all now use context. `i18n.js` was expanded to a complete en/am/om dictionary for every screen (cart review, confirm, payment methods, trust ladder, location, order-received popup, toasts). Regression checklist: run the full flow in am and om — intro → service → menu → category → cart review ("N items · review before confirming") → confirm → payment → "Almost there" → order popup; no screen may show English chrome.
- **Task 1 — intro screen cleaned.** The in-app language switcher and the "🛠️ Admin Panel" button were removed from `IntroFrame.jsx`; the Mini App renders in the bot-selected language from first paint (`user_langs` DB table → `?lang=` on every bot-sent URL, including the per-chat Menu Button). Regular customers see ordering UI only; admins reach the panel via `/admin`.
- **Task 2 — per-section category management.** `menu_categories` gained `section` ('food'|'drink', CHECK-enforced, idempotent ALTER for existing DBs) and `hidden` (disable without deleting items). Seed defaults: breakfast/snacks→food, hot_drinks/drinks→drink. In-bot portal: categories view split into independent 🍲/🥤 sections with per-section ➕ add, rename, hide/show, ⬆️⬇️ reorder **within its own section only**, and delete refused while items remain. Web admin gained a Categories page with the same operations + transactional per-section reorder endpoint (`POST /api/menu/categories/reorder`). Public `GET /api/menu/categories` now returns section+hidden; `menuData.js` sync groups server categories by section and drops hidden ones — admin edits appear in the Mini App **without redeploy**.
- **Task 4 — scammer blocklist.** New `blocked_users` table (tg_user_id PK, username, reason, blocked_by, blocked_at) + `isUserBlocked()`/`blockUser()`/`unblockUser()`/`findUserRef()` helpers in `db.js`. BOTH order entry points check the blocklist server-side BEFORE saving: `POST /api/miniapp/orders` returns 403 `{blocked:true}` (Mini App shows a localized "restricted — contact support" screen instead of the flow) and the bot's `web_app_data` path replies with a localized restriction message. Admin surfaces: bot ⚙️Settings → 🚫 Blocked users (list / block by ID or @username / unblock), web admin Users page (customer directory search, block/unblock), and a block/unblock action directly on each order record. Past orders stay visible to admin. DB errors fail open (never block all ordering).
- **Task 5 — `/admin` chooser.** Verified admins (server-side `isAdmin`: OWNER_ID + ADMIN_TELEGRAM_IDS + NOTIFY_CHAT_IDS) get an inline choice: "🤖 In-Bot Admin" (opens the existing in-place portal) vs "📊 Open Admin Panel" (web). Non-admins get the same generic localized denial as before. Also fixed a latent bug: `const lang` was reassigned in `ib_setlang_` (silently swallowed → alert-language picker never re-rendered); now `let`.
